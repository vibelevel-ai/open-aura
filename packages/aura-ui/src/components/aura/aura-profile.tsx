...
// ─── sparkline (last-N dimension scores; spec §1.2) ──────────────────────────
...
// Compact token count (M / K / plain) — shared so every Aura surface matches.
...
export function AuraProfile({
  ...
  // (build_profile) and surfaced next to "Avg of all sessions" on the
  // public/shared profile only (the dashboard is too tight). Shown when a real
  // best exists and it differs from the average (otherwise it's redundant).
  const showBest =
    !viewingSession &&
  ...
  // dashboard / sessions list link straight into one session's detail (no
