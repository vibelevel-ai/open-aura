...
@mcp.tool(
    annotations=ToolAnnotations(
        title="Compare my Aura to my cohort", readOnlyHint=True
    ),
    structured_output=False,  # placeholder shape (Phase 2) — keep it unstructured
)
async def compare_me() -> dict:
    """Return how the caller's Aura compares to their cohort (percentile/rank).

    WHEN to use: when the user asks "how do I rank?", "what percentile am I?",
    or "how do I compare to other builders?".

    POC STATUS: cohort definition (the "top X% across whom" basis) is a
    Phase-2 deferral (spec §9.7). This tool currently returns the caller's own
    Aura headline plus PLACEHOLDER comparison fields so the agent surface and
    client UI are wired end-to-end now. When real cohort logic lands, it plugs
    in HERE — replace the placeholder block below with a cohort/percentile query
    over the ``"AuraSession"`` sibling ranking (a separate referenceless basis;
    it must NOT blend with verified assessment scores — spec decision #10/#16).

    RETURNS: ``{"aura_score", "aura_level", "archetype", "percentile",
    "cohort", "cohort_size", "rank", "note"}``. While in POC, ``percentile`` /
    ``cohort`` / ``rank`` are null and ``note`` explains that ranking is coming
    in Phase 2.
    """
    user_id = current_user_id()
    profile = await build_profile(user_id)

    # --- Cohort logic plugs in here (Phase 2). For now, echo the user's own
    #     Aura headline with explicit null comparison fields. ---
    return {
        "aura_score": profile.get("aura_score"),
        "aura_level": profile.get("aura_level"),
        ...
        "rank": None,
        "note": (
            "Cohort ranking is coming in VibeLevel Aura Phase 2. Your Aura "
            "Score and archetype are shown above; percentile/rank will appear "
            "once the leaderboard cohort is defined."
        ),
    }
