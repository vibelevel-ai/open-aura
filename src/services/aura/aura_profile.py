...
async def whoami_summary(user_id: str) -> WhoAmIResponse:
    ...
    row from "User" plus a scored-session count. Used to confirm the agent's PAT
    resolves and to greet the user by name.
    """
    pool = None
    conn = None
    try:
...
async def aura_leaderboard(limit: int = 20) -> list[dict]:
    """Public Aura leaderboard — best + avg Aura score per public user.

    Sibling query (decision #10): ranks on the referenceless Aura basis only,
    never blended with the assessment leaderboard. Mirrors the assessment
    leaderboard's DISTINCT-ON-per-user + Python re-sort/limit shape and its
    exclusion style (only opt-in public, non-banned users).

    Each row also carries the user's BEST verified assessment level as a
    *badge* — `verified_level` / `verified_score` — LEFT JOINed from the
    graded `"AssessmentSession"` ranking. This is presentation only: the
    Aura rank is computed purely from Aura scores and is NEVER blended with
    the verified score (Aura v1.0 design: verified level is a badge, not a
    rank input).

    Returns rows shaped like AuraLeaderboardEntry (+ verified badge fields):
      {rank, handle, display_name, aura_score, avg_aura_score, aura_level,
       archetype, session_count, coding_sessions, noncoding_sessions,
       total_tokens, verified_level, verified_score}
    `aura_score` is the user's BEST scored session; `avg_aura_score` is also
    included for callers that want the averaged basis. `verified_level` /
    `verified_score` are null when the user has no eligible graded assessment.
    """
    pool = None
    conn = None
    try:
        ...
        # One row per public user: their best scored session (for archetype/
        # level display) plus their average across sessions, LEFT JOINed to
        # their best verified assessment level (badge only). Aura exclusions:
        # opt-in public visibility, scored sessions only, non-banned users.
        #
        # The verified-level subquery mirrors the assessment /leaderboard
        # exclusion filters (assessment_endpoints.py ~line 709): graded +
        # non-null overall score, public assessment, non-preview session
        # (session_mode IS NULL), non-banned user, approved review_status,
        # no medium/high external-assistance severity, identity not rejected.
        # DISTINCT ON picks each user's single best (highest overall) graded
        # session so verified_level reflects their peak verified result.
        cur.execute(
            """
            SELECT DISTINCT ON (s.user_id)
                   s.user_id,
                   s.aura_score   AS best_aura_score,
                   s.aura_level,
                   s.archetype,
                   s.modality,
                   s.cards,
                   avg_scores.avg_aura_score,
                   u.aura_handle,
                   u.aura_visibility,
                   u.display_name, u."firstName", u."lastName",
                   COALESCE(u.aura_likes_count, 0) AS like_count,
                   avg_scores.session_count,
                   avg_scores.coding_count,
                   avg_scores.primary_source,
                   verified.vl_level         AS verified_level,
                   verified.vl_score_overall AS verified_score
            FROM "AuraSession" s
            JOIN "User" u ON s.user_id = u.id
            JOIN (
                SELECT a.user_id,
                       AVG(a.aura_score) AS avg_aura_score,
                       COUNT(*) AS session_count,
                       COUNT(*) FILTER (WHERE a.modality = 'coding') AS coding_count,
                       mode() WITHIN GROUP (ORDER BY a.source) AS primary_source
                FROM "AuraSession" a
                WHERE a.status = 'scored' AND a.aura_score IS NOT NULL
                GROUP BY a.user_id
            ) avg_scores ON avg_scores.user_id = s.user_id
            LEFT JOIN (
                SELECT DISTINCT ON (vs.user_id)
                       vs.user_id, vs.vl_level, vs.vl_score_overall
                FROM "AssessmentSession" vs
                JOIN "Assessment" va ON vs.assessment_id = va.id
                WHERE vs.status = 'graded'
                  AND vs.vl_score_overall IS NOT NULL
                  AND va.is_public = true
                  AND vs.session_mode IS NULL
                  AND (vs.review_status IS NULL OR vs.review_status = 'approved')
                  AND (
                    vs.vl_feedback IS NULL
                    OR vs.vl_feedback->'behavior_feedback'->>'external_assistance_severity' IS NULL
                    OR vs.vl_feedback->'behavior_feedback'->>'external_assistance_severity' NOT IN ('medium', 'high', 'very_high')
                  )
                  AND (vs.identity_verdict IS NULL OR vs.identity_verdict != 'rejected')
                ORDER BY vs.user_id, vs.vl_score_overall DESC, vs.submitted_at ASC
            ) verified ON verified.user_id = s.user_id
            WHERE s.status = 'scored'
              AND s.aura_score IS NOT NULL
              AND u.aura_visibility = 'public'
              AND (u.is_banned IS NULL OR u.is_banned = false)
            ORDER BY s.user_id, s.aura_score DESC, s.created_at ASC
            """
        )
        rows = cur.fetchall()
        cur.close()

        # Rank by best Aura score desc (tiebreak: avg desc), then take top N.
        # Verified level is NOT a sort key — it is a badge, never blended.
        rows.sort(
            key=lambda r: (
                -(_to_float(r.get("best_aura_score")) or 0.0),
                -(_to_float(r.get("avg_aura_score")) or 0.0),
            )
        )
        rows = rows[:limit]

        # Token totals — computed the SAME way as the profile (/me/profile): from
        # each session's evidence via session_tokens, so the board never disagrees
        # with the profile. (The earlier card-stat SQL sum returned 0.) One batched
        # evidence fetch for the top-N users, then _compute_stats per user.
        tokens_by_user: dict[str, int] = {}
        top_user_ids = [r["user_id"] for r in rows if r.get("user_id")]
        if top_user_ids:
            cur2 = conn.cursor(cursor_factory=RealDictCursor)
            cur2.execute(
                'SELECT user_id, evidence FROM "AuraSession" '
                # user_id is uuid; the bound list is text, and uuid = ANY(text[])
                # has no operator — cast the column to text to compare.
                "WHERE user_id::text = ANY(%s) AND status = 'scored' "
                "AND aura_score IS NOT NULL",
                (top_user_ids,),
            )
            ev_by_user: dict[str, list] = {}
            for er in cur2.fetchall():
                ev_by_user.setdefault(er["user_id"], []).append(er)
            cur2.close()
            for uid, urows in ev_by_user.items():
                tokens_by_user[uid] = int(_compute_stats(urows).get("total_tokens", 0) or 0)

        results: list[dict] = []
        for i, row in enumerate(rows, 1):
            best = _to_float(row.get("best_aura_score"))
            avg = _to_float(row.get("avg_aura_score"))
            verified_score = _to_float(row.get("verified_score"))
            session_count = int(row.get("session_count") or 0)
            coding_sessions = int(row.get("coding_count") or 0)
            results.append(
                {
                    "rank": i,
                    "handle": row.get("aura_handle") or "",
                    "display_name": _display_name_from_row(row),
                    "aura_score": round(best, 1) if best is not None else 0.0,
                    "avg_aura_score": round(avg, 1) if avg is not None else 0.0,
                    "aura_level": row.get("aura_level"),
                    "archetype": row.get("archetype"),
                    # Best session's modality (coding/non-coding icon) + whether
                    # that session shipped/delivered end-to-end (lifecycle badge).
                    "modality": row.get("modality") or "",
                    "ships_it": _ships_it(row.get("cards")),
                    # Session telemetry: total scored sessions + coding split
                    # (non-coding = total - coding). Used by the leaderboard's
                    # Sessions column (coding vs non-coding breakdown).
                    "session_count": session_count,
                    "coding_sessions": coding_sessions,
                    "noncoding_sessions": max(session_count - coding_sessions, 0),
                    # Public count-only profile likes (shown on the leaderboard).
                    "like_count": int(row.get("like_count") or 0),
                    # Usage telemetry: total tokens across all the user's scored
                    # sessions (real-work volume) — computed from evidence above,
                    # matching the profile. Shown as the "Tokens" column.
                    "total_tokens": tokens_by_user.get(row.get("user_id"), 0),
                    # Primary agent = the user's most-used source across scored
                    # sessions (mode), shown as the "Agent" column on the board.
                    "source": row.get("primary_source"),
                    # Verified assessment badge (null when no eligible graded run).
                    "verified_level": row.get("verified_level"),
                    "verified_score": (
                        round(verified_score, 1) if verified_score is not None else None
                    ),
                }
            )
